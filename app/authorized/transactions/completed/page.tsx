...
export default function AuthorizedCompletedPage() {
  ...
  const loadData = useCallback(async () => {
    setError(null);
    try {
      const res = await fetch("/api/authorized/transactions?limit=200&status=DONE", { credentials: "include" });
      ...
      <header className="rounded-xl border border-[#2c3e50]/10 bg-white px-4 py-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:px-6">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl font-bold text-[#1a1a2e] sm:text-2xl">المعاملات المنجزة</h1>
            <p className="mt-1 text-sm text-[#5a5a5a]">المعاملات التي أكملتها من المعينة إليك</p>
          </div>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-[#2c3e50]/10 bg-white p-12 text-center shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
          ...
          <Link
            ...
            className="mt-4 inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-[#1E6B3A] px-6 py-3 text-sm font-medium text-white hover:bg-[#175a2e]"
          >
        ...
        <div className="grid gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {transactions.map((t) => (
            ...
            >
              <div className="flex flex-row flex-wrap items-center justify-between gap-2 border-b border-[#2c3e50]/8 bg-[#ccfbf1]/30 px-4 py-3">
                <span className="font-mono text-sm font-bold text-[#1E6B3A]" dir="ltr">
                  {t.serialNumber ? `2026-${t.serialNumber}` : "—"}
                </span>
                <span className="shrink-0 rounded-lg bg-[#ccfbf1] px-2.5 py-1 text-xs font-medium text-[#0f766e]">
                  منجزة
                </span>
              </div>
              <div className="flex flex-1 flex-col p-4">
                ...
                <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[#5a5a5a]">
                  ...
                  <span>تاريخ الإنجاز: {formatDate(t.completedAt)}</span>
                </p>
                <p className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                  <span className="flex items-center gap-1.5 text-[#5B7C99]">
                    ...
                    {(() => {
                      ...
                      return n === 0 ? "لا إجراءات" : n === 1 ? "إجراء واحد" : `${n} إجراءات`;
                    })()}
                  </span>
                ...
                <div className="mt-4 flex flex-row gap-2 border-t border-[#2c3e50]/8 pt-4">
                  <Link
                    ...
                    className="flex min-h-[44px] flex-1 items-center justify-center gap-2 rounded-xl border border-[#1E6B3A]/40 bg-[#1E6B3A]/5 px-4 py-3 text-sm font-medium text-[#1E6B3A] active:scale-[0.98] hover:bg-[#1E6B3A]/10"
                  >
