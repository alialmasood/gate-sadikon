...
export default function ReceptionReportsPage() {
  ...
  const PIE_COLORS = ["#1E6B3A", "#b91c1c", "#6b7280", "#1E6B3A", "#5B7C99", "#7C3AED", "#B08D57"];
  ...
  const exportToExcel = () => {
    ...
    a.href = url;
    a.download = `تقرير_معاملات_${dateFrom}_${dateTo}.csv`;
    a.click();
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex flex-col gap-4 border-b border-[#d4cfc8] pb-6 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#5a5a5a]">تقارير تفصيلية لمعاملات المكتب مع إمكانية التصدير</p>
        </div>
        <div className="flex flex-wrap items-end gap-3">
          <div>
            ...
            <input
              ...
              className="rounded-lg border border-[#d4cfc8] bg-white px-3 py-2 text-sm text-[#1B1B1B] focus:border-[#1E6B3A] focus:outline-none focus:ring-1 focus:ring-[#1E6B3A]/30"
            />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <>
          ...
          <article className="overflow-hidden rounded-2xl border border-[#d4cfc8] bg-white shadow-sm">
            ...
            <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-6">
              ...
              <div className="flex flex-col rounded-xl border border-[#d4cfc8] border-r-4 border-r-[#1E6B3A] bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 text-2xl font-bold text-[#1E6B3A]">{completionRate}%</p>
              </div>
              <div className="flex flex-col rounded-xl border border-[#d4cfc8] border-r-4 border-r-[#5B7C99] bg-white p-4 shadow-sm">
                <p className="text-sm font-medium text-[#5a5a5a]">معاملات منجزة من المدير</p>
                <p className="mt-2 text-2xl font-bold text-[#1B1B1B]">
                  {transactions.filter((t) => t.completedByAdmin).length}
                </p>
              </div>
            </div>
          </article>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {/* رسم بياني: المعاملات المسجلة يومياً */}
            <article className="overflow-hidden rounded-2xl border border-[#d4cfc8] bg-white shadow-sm">
              <div className="border-b border-[#d4cfc8] bg-[#f6f3ed]/50 px-6 py-3">
                ...
                {dailyData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height={288} minHeight={200}>
                    <BarChart data={dailyData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                      ...
                      />
                      <Bar dataKey="count" fill="#1E6B3A" radius={[4, 4, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
                    ...
                    <table className="w-full min-w-[300px] text-right text-sm">
                      <thead>
                        <tr className="border-b border-[#d4cfc8] bg-[#f6f3ed]/50">
                          <th className="py-2 px-3 font-medium text-[#5a5a5a]">نوع المعاملة</th>
                          <th className="py-2 px-3 font-medium text-[#5a5a5a]">العدد</th>
                          <th className="py-2 px-3 font-medium text-[#5a5a5a]">النسبة</th>
                        </tr>
                      </thead>
                      <tbody>
                        {typeData.map(({ name, value }) => (
                          <tr key={name} className="border-b border-[#d4cfc8]/80">
                            <td className="py-2 px-3 font-medium text-[#1B1B1B]">{name}</td>
                            <td className="py-2 px-3 text-[#1E6B3A]">{value}</td>
                            <td className="py-2 px-3 text-[#5a5a5a]">{total > 0 ? Math.round((value / total) * 100) : 0}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <ul className="space-y-2">
                    {workflowData.map((item, i) => (
                      <li key={item.name} className="flex items-center justify-between rounded-lg border border-[#d4cfc8] bg-[#f6f3ed]/30 px-4 py-3">
                        ...
                        <span className="rounded-full bg-[#1E6B3A]/10 px-3 py-0.5 text-sm font-medium text-[#1E6B3A]">
                          {item.value}
            ...
            <article className="overflow-hidden rounded-2xl border border-[#d4cfc8] bg-white shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[600px] text-right text-sm">
                  <thead>
                    <tr className="border-b border-[#d4cfc8] bg-[#f6f3ed]/50">
                      <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">رقم المعاملة</th>
                      <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">المواطن</th>
                      <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">نوع المعاملة</th>
                      <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">تاريخ التقديم</th>
                      <th className="py-2 px-3 font-medium text-[#5a5a5a]">إجراء</th>
                    </tr>
                  </thead>
                  <tbody>
                    {transactions
                      ...
                      .map((t) => (
                        <tr key={t.id} className="border-b border-[#d4cfc8]/80">
                          <td className="border-l border-[#d4cfc8]/60 py-2 px-3 font-mono text-[#1E6B3A]">{t.serialNumber || "—"}</td>
                          <td className="border-l border-[#d4cfc8]/60 py-2 px-3 font-medium text-[#1B1B1B]">{t.citizenName || "—"}</td>
                          <td className="border-l border-[#d4cfc8]/60 py-2 px-3 text-[#5a5a5a]">{t.transactionType || t.type || "—"}</td>
                          <td className="border-l border-[#d4cfc8]/60 py-2 px-3 text-[#5a5a5a]">{formatDateShort(t.submissionDate || t.createdAt)}</td>
                          <td className="py-2 px-3">
                            <Link href="/reception/citizens" className="text-[#1E6B3A] underline hover:no-underline">
                              عرض
                            </Link>
                          </td>
                        </tr>
          ...
          <article className="overflow-hidden rounded-2xl border border-[#d4cfc8] bg-white shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[800px] text-right text-sm">
                <thead>
                  <tr className="border-b border-[#d4cfc8] bg-[#f6f3ed]/50">
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">رقم</th>
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">المواطن</th>
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">النوع</th>
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">الحالة</th>
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">مسار العمل</th>
                    <th className="border-l border-[#d4cfc8] py-2 px-3 font-medium text-[#5a5a5a]">تاريخ التقديم</th>
                    <th className="py-2 px-3 font-medium text-[#5a5a5a]">إجراء</th>
                  </tr>
                </thead>
                <tbody>
                  {transactions.slice(0, 15).map((t) => (
                    <tr key={t.id} className="border-b border-[#d4cfc8]/80">
                      <td className="border-l border-[#d4cfc8]/60 py-2 px-3 font-mono text-[#1E6B3A]">{t.serialNumber || "—"}</td>
                      <td className="border-l border-[#d4cfc8]/60 py-2 px-3 font-medium text-[#1B1B1B]">{t.citizenName || "—"}</td>
                      <td className="max-w-[120px] truncate border-l border-[#d4cfc8]/60 py-2 px-3 text-[#5a5a5a]" title={t.transactionType || t.type || ""}>
                        {t.transactionType || t.type || "—"}
                      </td>
                      <td className="border-l border-[#d4cfc8]/60 py-2 px-3">
                        <span className="rounded-full bg-[#1E6B3A]/10 px-2 py-0.5 text-xs font-medium text-[#1E6B3A]">
                          {STATUS_LABELS[t.status] ?? t.status}
                        </span>
                      </td>
                      <td className="border-l border-[#d4cfc8]/60 py-2 px-3 text-[#5a5a5a]">{WORKFLOW_LABELS[getWorkflowStatus(t)] ?? "—"}</td>
                      <td className="border-l border-[#d4cfc8]/60 py-2 px-3 text-[#5a5a5a]">{formatDateShort(t.submissionDate || t.createdAt)}</td>
                      <td className="py-2 px-3">
                        <Link
                          href={`/reception/citizens?sn=${t.serialNumber || ""}`}
                          className="text-[#1E6B3A] underline hover:no-underline"
                        >
                          عرض
                        </Link>
                      </td>
