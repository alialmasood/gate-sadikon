...
function getBreadcrumb(pathname: string): string[] {
  ...
  if (pathname in map) return ["المدقق", map[pathname]];
  return ["المدقق", "لوحة التحكم"];
}

const NAV_ITEMS = [{ href: "/auditor", label: "لوحة التحكم" }];

const SIDEBAR_COLLAPSED_KEY = "auditor-sidebar-collapsed";
          ...
          <div className="flex flex-1 flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="hidden items-center gap-2 text-sm text-[#5a5a5a] sm:flex">
                <Link href="/auditor" className="hover:text-[#7C3AED]">{breadcrumb[0]}</Link>
                <span>/</span>
