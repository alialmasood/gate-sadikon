  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [citizensRes, transRes] = await Promise.all([
        fetch("/api/super-admin/citizens"),
        fetch("/api/super-admin/transactions?limit=100"),
      ]);
      if (citizensRes.ok) {
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <article className="rounded-2xl border border-[#d4cfc8] bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#5a5a5a]">المواطنين الذين قدموا معاملات مع إحصائيات معاملاتهم.</p>
        {loading ? (
      ...
      <article className="rounded-2xl border border-[#d4cfc8] bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#5a5a5a]">
          {selectedCitizen ? `معاملات المواطن المحدد` : "جميع المعاملات المسجلة. اضغط على مواطن في القائمة أعلاه لفلترة معاملاته."}
        </p>
        {loading ? (
          <p className="mt-4 py-8 text-center text-[#5a5a5a]">جاري التحميل…</p>
          ...
          <div className="mt-4 overflow-x-auto">
              ...
              <tbody>
                {filteredTransactions.slice(0, 50).map((t) => (
                  <tr key={t.id} className="border-b border-[#d4cfc8]/80">
                    ...
                    <td className="py-3 pr-2 text-[#5a5a5a]">{t.office?.name || "—"}</td>
                    <td className="py-3 pr-2">
                      <span
            ...
            {filteredTransactions.length > 50 && (
              <p className="mt-3 text-sm text-[#5a5a5a]">عرض 50 من أصل {filteredTransactions.length} معاملة</p>
            )}
          </div>
        )}
      </article>
      ...
      <article className="rounded-2xl border border-[#d4cfc8] bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#5a5a5a]">إجمالي المعاملات حسب الحالة.</p>
